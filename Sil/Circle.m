function [C,R] =  Circle (Img, p , N , CORRS, CORRS2, epsilon)
...
%  if (size(Img,1)/size(Img,2) < abs(m))
     S_x = round((S_y-p(1)+ (m*p(2)) )/m);
%         if () 
%  else
     S_y = round ( (m*(S_x-p(2)))+ p(1));
%  end

plot(S_y,S_x,'*green','MarkerSize',10);


S_x
S_y
p_t = zeros(1,2);
[p_t(1),p_t(2)] = ind2sub(size(S2),CORRS2(S_y,S_x));    %%Correct Form
...
if (Img(c_i(2),c_i(1)) == 1)          % IF THE CENTER LIES INSIDE OF THE IMAGE
...
while (abs(rho_i - rho_i1) > epsilon)
    ...
    if (Img(c_i(1),c_i(2)) == 1)          % IF THE CENTER LIES INSIDE OF THE IMAGE
